...
try {
    ...
    for ($batch_start = 0; $batch_start < $total_bills; $batch_start += $batch_size) {
        ...
        // ============================================================================
        // STEP 10.6: RECORD CUSTOMER SALES IN tblcustomersales
        // ============================================================================
        // Debug: Log all session variables related to customer
        logMessage("DEBUG: Session check in generate_bills_ultra_fast - selected_customer: " . (isset($_SESSION['selected_customer']) ? $_SESSION['selected_customer'] : 'NOT SET'), 'DEBUG');
        logMessage("DEBUG: All relevant session vars: " . json_encode([
            'selected_customer' => $_SESSION['selected_customer'] ?? 'NOT SET',
            'customer_prices' => isset($_SESSION['customer_prices']) ? count($_SESSION['customer_prices']) : 0
        ]), 'DEBUG');
        
        // Get selected customer from session
        $selectedCustomer = isset($_SESSION['selected_customer']) ? (int)$_SESSION['selected_customer'] : 0;
        
        logMessage("DEBUG: After casting - selectedCustomer: $selectedCustomer", 'DEBUG');
        
        if ($selectedCustomer > 0) {
            logMessage("Recording customer sales for customer ID: $selectedCustomer", 'INFO');
            
            foreach ($batch_bills as $bill) {
                // Extract numeric bill number from bill_no (e.g., "BL0001" -> 1)
                $numeric_bill_no = (int)substr($bill['bill_no'], 2);
                ...
                foreach ($bill['items'] as $item) {
                    ...
                    // Debug log
                    logMessage("Recording customer sale - BillNo: $numeric_bill_no, Item: {$item['code']}, Qty: $quantity, Rate: $rate", 'DEBUG');
                    ...
                    if (!$customerSaleResult) {
                        logMessage("WARNING: Failed to record customer sale for bill {$bill['bill_no']}, item {$item['code']}", 'WARNING');
                    }
                }
            }
        } else {
            logMessage("No customer selected - skipping customer sales recording", 'INFO');
        }
