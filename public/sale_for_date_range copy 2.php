...
function getNextBillNumber($conn) {
    // Use a more robust method to get the next bill number
    $query = "SELECT MAX(CAST(SUBSTRING(BILL_NO, 3) AS UNSIGNED)) as max_bill FROM tblsaleheader";
    $result = $conn->query($query);
    $row = $result->fetch_assoc();
    $max_bill = $row['max_bill'] ? (int)$row['max_bill'] : 0;
    
    // Add 1 to get the next bill number
    return $max_bill + 1;
}
...
function generateBillsWithLimits($conn, $items_data, $date_array, $daily_sales_data, $mode, $comp_id, $user_id, $fin_year_id) {
    ...
    // Get the maximum bill number and set next bill
    $query = "SELECT MAX(CAST(SUBSTRING(BILL_NO, 3) AS UNSIGNED)) as max_bill FROM tblsaleheader";
    $result = $conn->query($query);
    $row = $result->fetch_assoc();
    $next_bill = $row['max_bill'] ? (int)$row['max_bill'] + 1 : 1;
...
// Handle form submission for sales update (updated section)
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['update_sales'])) {
        ...
        try {
            ...
            $bills = generateBillsWithLimits($conn, $items_data, $date_array, $daily_sales_data, $mode, $comp_id, $user_id);
...
function generateBills() {
    ...
    // Send AJAX request
    $.ajax({
        url: 'generate_bills.php', // This file needs to be updated with the new logic
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
