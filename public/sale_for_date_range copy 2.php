...
// NEW: Enhanced distribution function that distributes only on available dates
function distributeSalesOnAvailableDates(totalQty, availableDates) {
...
function updateDistributionPreviewWithAvailableDates(itemCode, totalQty) {
    ...
    if (hasBackdatedRestriction && availableDates.length > 0) {
        // Item has existing sales on some dates - distribute only on available dates
        console.log(`Item ${itemCode}: Distributing ${totalQty} on available dates:`, availableDates);
        dailySales = distributeSalesOnAvailableDates(totalQty, availableDates);
    ...
    dailySales.forEach((qty, index) => {
        ...
        if (isUnavailable) {
            // Date has existing sales - no new sales allowed
            cell.addClass('unavailable-date-cell');
            cell.attr('title', `Sales already exist on ${date} - No new sales allowed`);
            
            // Add visual indicator (✗ symbol)
            cell.html('<span class="text-danger">✗</span>');
            
        } else if (isAvailable && qty > 0) {
            // Date is available and has new sales
            cell.addClass('available-date-cell non-zero-distribution');
    ...
    const totalDistributed = dailySales.reduce((sum, qty) => sum + qty, 0);
...
$(document).ready(function() {
    ...
    $('#shuffleBtn').off('click').on('click', async function() {
        ...
        for (const item of itemsToShuffle) {
            updateDistributionPreviewWithAvailableDates(item.itemCode, item.totalQty);
        }
