...
try {
    ...
    // Check stock for each item with quantity > 0
    foreach ($quantities as $item_code => $total_qty) {
        if ($total_qty > 0) {
            foreach ($date_array as $sale_date) {
                ...
                if ($check_result->num_rows == 0) {
                    ...
                    if ($current_stock < $total_qty) {
                        $validation_errors[] = "Insufficient stock for item $item_code on $sale_date. Available: $current_stock, Required: $total_qty";
                    }
                }
                $check_stmt->close();
