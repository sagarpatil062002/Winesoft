...
// ============================================================================
// EXCEL PROCESSING FUNCTION (Handles both real Excel and HTML-format .xls)
// ============================================================================
...
function processSingleExcelFile($filePath, $companyId, $conn, $importMode, $defaultStatus, $updateMRP, $updateStockFlag, $allowed_classes, $fileName = '') {
    ...
    // Check if file is actually HTML (many .xls exports are HTML tables)
    $fileHeader = file_get_contents($filePath, false, null, 0, 512);
    $isHtml = preg_match('/<html|<!DOCTYPE\s+html|<table/i', $fileHeader);
    
    if ($isHtml) {
        debugLog("Excel file detected as HTML format, using regex parser");
        return processHtmlExcelFile($filePath, $companyId, $conn, $importMode, $defaultStatus, $updateMRP, $updateStockFlag, $allowed_classes, $fileName);
    }
    
    // Real Excel file - use PhpSpreadsheet
...
// ============================================================================
// HTML EXCEL FILE PROCESSING (Parses HTML table directly with regex)
// ============================================================================

function processHtmlExcelFile($filePath, $companyId, $conn, $importMode, $defaultStatus, $updateMRP, $updateStockFlag, $allowed_classes, $fileName = '') {
    debugLog("Processing HTML-format Excel file: " . $fileName);
    ...
    // Extract ALL <td> cell contents using regex
    if (!preg_match_all('/<td[^>]*>(.*?)<\/td>/si', $htmlContent, $cellMatches)) {
        return [
            'successCount' => 0,
            'errorCount' => 1,
            'errors' => ["No table data found in HTML file"]
        ];
    }
    ...
    debugLog("Extracted $totalCells cells from HTML");
    
    // Structure:
    // Cell 0: colspan header "Licensee: ..."
    // Cell 1: colspan header "TP Details"
    // Cells 2-28: Column headers (27 columns)
    // Cells 29+: Data rows (27 columns each)
    $colsPerRow = 27;
    $colspanCells = 2;
    
    // Extract column headers
    $rawHeaders = array_slice($allCells, $colspanCells, $colsPerRow);
    ...
    debugLog("Column headers from HTML:", $headers);
    
    // Convert data cells to rows array
    $dataCells = array_slice($allCells, $colspanCells + $colsPerRow);
    $dataRows = [];
    for ($i = 0; $i < count($dataCells); $i += $colsPerRow) {
        $row = array_slice($dataCells, $i, $colsPerRow);
        $dataRows[] = $row;
    }
    
    debugLog("Extracted " . count($dataRows) . " data rows from HTML");
    
    if (empty($dataRows)) {
        return [
            'successCount' => 0,
            'errorCount' => 1,
            'errors' => ["No data rows found in HTML file"]
        ];
    }
    
    // Build TP groups from data rows
    $tpGroups = [];
    $processedRows = 0;
    $rowsWithTp = 0;
    ...
    foreach ($dataRows as $row) {
        ...
        debugLog("Row $processedRows - Added to TP $tpNo: $itemName, Cases: $cases, Bottles: $bottles");
    }
    
    debugLog("HTML Processing Summary", [
        'total_rows' => $processedRows,
        'rows_with_tp' => $rowsWithTp,
        'tp_groups' => count($tpGroups)
    ]);
    ...
    if (count($tpGroups) == 0) {
        return [
            'successCount' => 0,
            'errorCount' => 1,
            'errors' => ["No valid data found in HTML file. Rows processed: $processedRows"]
        ];
    }
